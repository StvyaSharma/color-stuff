...
export function perceivedColor(
  ...
  try {
    const perceivedChroma = chroma.lab(perceivedL, perceivedA, perceivedB)
...
export function subtractColor(
  ...
  try {
    const subtractedChroma = chroma.lab(subtractedL, subtractedA, subtractedB)
      .alpha(baseColor.alpha);
...
export function adjustBoundaries(
  ...
  try {
    const adjustedChroma = chroma.lab(adjustedL, baseLab[1], baseLab[2]).alpha(
      baseColor.alpha,
    );
...
export function modifyEmphasis(
  ...
  try {
    ...
    const adjustedChroma = chroma.lch(baseLCH[0], adjustedC, hue).alpha(
      baseColor.alpha,
    );
    return toIColor(adjustedChroma);
  } catch (e) {
